        ...
        <main style={{ maxWidth: 900, margin: '0 auto', padding: '56px 5%' }}>
                ...
                <div style={{ marginBottom: 32 }}>
                        ...
                        {TABS.map(tab => {
                            ...
                            return (
                                <button key={tab.key} onClick={() => setActiveTab(tab.key)}
                                    ...
                                    <span style={{ minWidth: 20, textAlign: 'center', fontSize: '0.66rem', fontWeight: 700, padding: '1px 6px', borderRadius: 'var(--r-full)', background: active ? 'var(--accent)' : count > 0 ? 'var(--surface-3)' : 'transparent', color: active ? '#fff' : count > 0 ? 'var(--text-3)' : 'var(--text-4)', transition: 'all 0.2s' }}>{count}</span>
                                </button>
                            );
                        })}
                    </div>
                    {posts.length > 0 && (
                        <div style={{ marginTop: 16, display: 'flex', alignItems: 'center', gap: 8 }}>
                            ...
                            <span style={{ fontSize: '0.78rem', fontWeight: 600, color: 'var(--text-2)' }}>{activeTabInfo?.icon} {activeTab === 'all' ? 'All Posts' : activeTabInfo?.label}</span>
            ...
            {!loading && posts.length > 0 && filtered.length === 0 && (
                <div style={{ textAlign: 'center', padding: '48px 24px', background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 'var(--r-xl)' }}>
                    ...
                    <p style={{ color: 'var(--text-3)', marginBottom: 20, fontSize: '0.88rem' }}>Write a post and select <strong>{activeTabInfo?.label}</strong> as the category.</p>
                    ...
                    {filtered.map(post => {
                        ...
                        return (
                            <div key={post._id}
                                ...
                                <div onClick={() => navigate(`/posts/${post._id}`)} style={{ width: 110, minHeight: 110, flexShrink: 0, cursor: 'pointer', overflow: 'hidden', background: 'var(--surface-3)' }}>
                                    {postImg
                                        ? <img src={postImg} alt={post.title} style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block' }} />
                                ...
                                <div style={{ flex: 1, padding: '16px 20px', minWidth: 0, display: 'flex', flexDirection: 'column', justifyContent: 'center', gap: 5 }}>
                                    <div style={{ display: 'flex', alignItems: 'center', gap: 7, flexWrap: 'wrap' }}>
                                        ...
                                        {post.category ? <span style={{ fontSize: '0.62rem', fontWeight: 700, padding: '2px 9px', borderRadius: 'var(--r-full)', background: 'var(--accent-light)', color: 'var(--accent)', display: 'inline-flex', alignItems: 'center', gap: 4 }}>{TABS.find(t => t.key === post.category)?.icon} {post.category}</span> : <span style={{ fontSize: '0.62rem', fontWeight: 600, padding: '2px 9px', borderRadius: 'var(--r-full)', background: 'var(--surface-3)', color: 'var(--text-4)' }}>Uncategorized</span>}
                                    </div>
                                ...
                                <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', gap: 8, padding: '16px 18px', flexShrink: 0 }}>
                                    {[{ label: 'View', danger: false, action: () => navigate(`/posts/${post._id}`) }, { label: 'Edit', danger: false, action: () => navigate(`/edit-post/${post._id}`) }, { label: 'Delete', danger: true, action: () => handleDelete(post._id) }].map(btn => (
                                        <button key={btn.label} onClick={btn.action}
                                            ...
                                            onMouseEnter={e => { if (btn.danger) { e.currentTarget.style.background = 'var(--red)'; e.currentTarget.style.color = '#fff'; } else { e.currentTarget.style.borderColor = 'var(--accent)'; e.currentTarget.style.color = 'var(--accent)'; } }}
                                            onMouseLeave={e => { if (btn.danger) { e.currentTarget.style.background = 'rgba(179,58,58,0.06)'; e.currentTarget.style.color = 'var(--red)'; } else { e.currentTarget.style.borderColor = 'var(--border)'; e.currentTarget.style.color = 'var(--text-2)'; } }}>
                                            {btn.label}
                                        </button>
                                    ))}
                                </div>
