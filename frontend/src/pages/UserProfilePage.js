...
export default function UserProfilePage() {
    ...
    // Move handleFollow to useCallback so it's a stable dependency if needed
    ...
    useEffect(() => {
        // Redirect if this is actually the current user's own profile
        if (me && me._id === id) { 
            navigate('/profile'); 
            return; 
        }
        ...
        const fetchProfileData = async () => {
            ...
            try {
                ...
                setPosts(postsRes.data.filter(post => post.author?._id === id));

                // Fetch follow status/stats separately to ensure fresh data
                if (me) {
                    const followRes = await API.get(`/follow/${id}`);
                    setFollowing(followRes.data.following);
                    setFollowerCount(followRes.data.followerCount);
                    setFollowingCount(followRes.data.followingCount);
                } else {
                    const statsRes = await API.get(`/follow/${id}/stats`);
        ...
        fetchProfileData();
    }, [id, me, navigate]); // Added navigate as a dependency
        ...
        <main style={{ background: 'var(--bg)', minHeight: '100%' }}>
                ...
                <div style={{ maxWidth: 860, margin: '0 auto' }}>
                    ...
                    <div style={{ display: 'flex', gap: 28, alignItems: 'flex-start', flexWrap: 'wrap' }}>
                        <div style={{ position: 'relative', flexShrink: 0 }}>
                            {profile.profilePic
                                ? <img src={`http://localhost:5000/uploads/${profile.profilePic}`} alt={profile.name} style={{ width: 100, height: 100, borderRadius: '50%', objectFit: 'cover', border: '3px solid var(--surface)', boxShadow: 'var(--sh-md)' }} />
                        ...
                        <div style={{ flex: 1, minWidth: 200 }}>
                            ...
                            <p style={{ fontSize: '0.78rem', color: 'var(--text-4)', marginBottom: 20 }}>
            ...
            <div style={{ borderBottom: '1px solid var(--border)', background: 'var(--surface)' }}>
                <div style={{ maxWidth: 860, margin: '0 auto', padding: '0 5%', display: 'flex' }}>
                    {[
                        { key: 'posts',     label: `Stories (${publishedPosts.length})` },
                ...
                {activeTab === 'posts' && (
                    publishedPosts.length === 0 ? (
                        <div style={{ textAlign: 'center', padding: '56px 24px', background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 'var(--r-xl)' }}>
                            ...
                            <h3 style={{ fontFamily: 'var(--font-display)', fontStyle: 'italic', fontSize: '1.5rem', marginBottom: 8 }}>No stories yet</h3>
                            <p style={{ color: 'var(--text-3)', fontSize: '0.9rem' }}>{profile.name} hasn't published any stories yet.</p>
                        </div>
                    ) : (
                    ...
                    <div key={tabKey}>
                        {(profile[tabKey] || []).length === 0 ? (
                            <div style={{ textAlign: 'center', padding: '56px 24px', background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 'var(--r-xl)' }}>
                                ...
                                <h3 style={{ fontFamily: 'var(--font-display)', fontStyle: 'italic', fontSize: '1.5rem', marginBottom: 8 }}>
                                    {tabKey === 'followers' ? 'No followers yet' : 'Not following anyone yet'}
                                </h3>
                                <p style={{ color: 'var(--text-3)', fontSize: '0.9rem' }}>
                                    {tabKey === 'followers' ? `Be the first to follow ${profile.name}!` : `${profile.name} hasn't followed any writers yet.`}
                                </p>
                            </div>
                        ) : (
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(260px, 1fr))', gap: 14 }}>
                                {(profile[tabKey] || []).map(u => (
                                    <div key={u._id} onClick={() => navigate(`/user/${u._id}`)}
                                        ...
                                        {u.profilePic
                                            ? <img src={`http://localhost:5000/uploads/${u.profilePic}`} alt={u.name} style={{ width: 44, height: 44, borderRadius: '50%', objectFit: 'cover', flexShrink: 0 }} />
