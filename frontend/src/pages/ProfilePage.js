...
export default function ProfilePage() {
    ...
    const handleDeleteAccount = async () => {
        try { await API.post('/auth/deactivate', { password: deletePw }); logout(); }
    ...
    // Use getImageUrl for both existing profile pic and local preview
    const picSrc = picPreview || getImageUrl(user?.profilePic);
    ...
    return (
        <main style={{ maxWidth: 1100, margin: '0 auto', padding: '56px 5%' }}>
            ...
            {msg && (
                <div style={{ background: msgType === 'error' ? 'rgba(179,58,58,0.08)' : 'rgba(45,106,79,0.08)', border: `1px solid ${msgType === 'error' ? 'rgba(179,58,58,0.25)' : 'rgba(45,106,79,0.25)'}`, borderRadius: 'var(--r-md)', padding: '12px 18px', marginBottom: 24, color: msgType === 'error' ? 'var(--red)' : 'var(--green)', fontSize: '0.88rem' }}>{msg}</div>
            )}
                    ...
                    {!editMode ? (
                        <div style={{ display: 'grid', gridTemplateColumns: 'auto 1fr', gap: '24px 40px', alignItems: 'start' }}>
                            ...
                            <div style={{ display: 'grid', gap: 16 }}>
                                {[{ label: 'Display Name', value: user?.name }, { label: 'Email', value: user?.email }, { label: 'Bio', value: user?.bio || '—' }, { label: 'Role', value: user?.role }, { label: 'Member since', value: user?.createdAt ? new Date(user.createdAt).toLocaleDateString('en-US', { month: 'long', year: 'numeric' }) : '—' }].map(f => (
                                    <div key={f.label}>
                            ...
                            return (
                                <div key={post._id} style={{ display: 'flex', gap: 16, padding: '14px 0', borderBottom: '1px solid var(--border-2)', alignItems: 'center' }}>
                                    <div style={{ width: 56, height: 56, borderRadius: 'var(--r-md)', background: 'var(--surface-3)', overflow: 'hidden', flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 20 }}>
                                        {postImg ? <img src={postImg} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : '✒️'}
                                    </div>
            ...
            {tab === 'Password' && (
                <div style={S.card}>
                    ...
                    <form onSubmit={handlePasswordChange} style={{ maxWidth: 440, display: 'flex', flexDirection: 'column', gap: 24 }}>
                        {[{ label: 'Current Password', val: curPw, set: setCurPw, ph: 'Enter current password' }, { label: 'New Password', val: newPw, set: setNewPw, ph: 'Min 6 characters' }, { label: 'Confirm New Password', val: cfmPw, set: setCfmPw, ph: 'Repeat new password' }].map(f => (
                            <div key={f.label}>
                        ...
                        {newPw && cfmPw && <p style={{ fontSize: '0.76rem', fontWeight: 600, color: newPw === cfmPw ? 'var(--green)' : 'var(--red)', marginTop: -12 }}>{newPw === cfmPw ? '✓ Passwords match' : '✗ Do not match'}</p>}
                ...
                <div style={S.card}>
                    ...
                    <div style={{ padding: '24px 0', borderBottom: '1px solid var(--border-2)', marginBottom: 32 }}>
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
                            {[{ label: 'Member since', value: user?.createdAt ? new Date(user.createdAt).toLocaleDateString('en-US', { month: 'long', year: 'numeric' }) : '—' }, { label: 'Account role', value: user?.role }, { label: 'Total posts', value: posts.length }, { label: 'Account status', value: 'Active' }].map(f => (
